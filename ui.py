...
async function openReview(mid){const box=$('wlreview');if(!box)return;
 ...
 const m=d.member||{};
 const hist=(d.hist||[]).map(h=>`<div><span class="dot ${h.cls}">•</span> <span class="muted">${esc(h.date||'—')}</span> ${esc(h.event_type)} — ${esc(String(h.detail||'').slice(0,100))}</div>`).join('')||'<span class="muted">no history</span>';
